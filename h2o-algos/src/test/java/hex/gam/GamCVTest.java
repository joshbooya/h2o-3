...
// the predictions kept from cross-validation should yield the same result!  
  ...
  public void testCVBinomial() {
    ...
    Frame[] predFrames;
    Frame[] cvModelPreds;
    Frame fold_assignment_frame;

    try {
      Scope.enter();
      train = parse_test_file("smalldata/glm_test/binomial_20_cols_10KRows.csv");
      ...
      cv_models = new GAMModel[params._nfolds];
      cvModelPreds = new Frame[params._nfolds];
      predFrames = new Frame[params._nfolds];
      fold_assignment_frame = Scope.track((Frame)DKV.getGet(gam._output._cross_validation_fold_assignment_frame_id));
